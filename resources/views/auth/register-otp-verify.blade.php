<!DOCTYPE html>
<html lang="fa" dir="rtl" class="light-style customizer-hide">
        ...
        <div class="authentication-inner row m-0">
            ...
            <div class="d-flex col-12 col-lg-5 col-xl-4 align-items-center authentication-bg p-4 p-sm-5">
                <div class="w-px-400 mx-auto">
                    ...
                    <h5 class="mb-3" style="text-align:center">تایید شماره موبایل</h5>
                    ...
                    <div class="text-center mt-4">
                        زمان باقیمانده:
                        <span id="timer" class="countdown text-warning">02:00</span>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            inputs.forEach((input, index) => {
                input.addEventListener('input', function() {
                    ...
                    hiddenInput.value = [...inputs].map(item => item.value).join('');
                });
            ...
            function startTimer() {
                clearInterval(timerInterval);
                updateTimer(); // اولین بروزرسانی فوری
            ...
            resendBtn.addEventListener('click', function() {
                ...
                fetch("{{ route('register.otp.resend') }}", {
                    ...
                    .then(data => {
                        if (data.success) {
                            // تبدیل امن به عدد (ثانیه)
                            let newExpiresAt = parseInt(data.expires_at, 10);

                            // اگر NaN بود (یعنی رشته غیرقابل تبدیل)، آن را با Date.parse امتحان کن
                            if (isNaN(newExpiresAt) && typeof data.expires_at === 'string') {
                                newExpiresAt = Math.floor(new Date(data.expires_at).getTime() / 1000);
                            }

                            // اگر باز هم نامعتبر بود، خطا بده
                            if (isNaN(newExpiresAt) || newExpiresAt <= 0) {
                                console.error('expires_at نامعتبر است:', data.expires_at);
                                showAlert('خطا در بروزرسانی تایمر', 'danger');
                                resendBtn.disabled = false;
                                resendBtn.innerHTML = 'ارسال مجدد کد';
                                return;
                            }

                            // تایمر قبلی را کاملاً متوقف کن
                            clearInterval(timerInterval);

                            // مقدار جدید را جایگزین کن
                            expiresAt = newExpiresAt;

                            // دکمه را مخفی کن و متن را برگردان
                            ...
                            updateTimer(); // بلافاصله نمایش را به‌روز کن
                            timerInterval = setInterval(updateTimer, 1000);
                        } else {
                    ...
                    .catch(error => {
                        console.error(error);
                        showAlert('خطا در ارسال مجدد کد', 'danger');
                        resendBtn.disabled = false;
                        resendBtn.innerHTML = 'ارسال مجدد کد';
                    });
            });
            ...
            function showAlert(message, type = 'success') {
                ...
                }, 5000); // زمان ۵ ثانیه (قبلاً اشتباهاً ۵۰۰ ثانیه بود)
            }
        });
    </script>
</body>

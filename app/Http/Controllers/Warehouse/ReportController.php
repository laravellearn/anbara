...
class ReportController extends BaseController
{
    ...
    // ─── ۵. خروجی Excel سفارشات خرید ────────────────────────────────────────
    public function purchaseOrdersExport(Request $request)
    {
        Gate::authorize('access', 'purchase-orders.view');

        [$tenantId, $companyId] = $this->tenantCtx();

        $query = \Illuminate\Support\Facades\DB::table('purchase_orders as po')
            ->leftJoin('contacts as sup', 'sup.id', '=', 'po.supplier_id')
            ->leftJoin('warehouses as wh', 'wh.id', '=', 'po.warehouse_id')
            ->leftJoin('users as u', 'u.id', '=', 'po.created_by')
            ->where('po.tenant_id', $tenantId)
            ->where('po.company_id', $companyId)
            ->whereNull('po.deleted_at')
            ->select([
                'po.po_number as شماره_سفارش',
                'po.order_date as تاریخ_سفارش',
                'po.status as وضعیت',
                'sup.full_name as تأمین_کننده',
                'wh.title as انبار',
                'po.currency as ارز',
                'po.discount_percent as تخفیف_درصد',
                'po.tax_percent as مالیات_درصد',
                'po.shipping_cost as هزینه_حمل',
                'po.notes as یادداشت',
                'u.name as ثبت_کننده',
                'po.created_at as تاریخ_ثبت',
            ]);
        ...
        $rows = $query->orderByDesc('po.order_date')->get();

        // ترجمه وضعیت
        $statusLabels = \App\Models\PurchaseOrder::statusLabels();
        $rows = $rows->map(function ($r) use ($statusLabels) {
            $r->وضعیت = $statusLabels[$r->وضعیت] ?? $r->وضعیت;
            return $r;
        });

        return $this->exportExcel($rows, 'purchase-orders', 'گزارش سفارشات خرید');
    }

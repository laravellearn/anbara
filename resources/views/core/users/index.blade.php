                ...
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        <div class="content-left">
                            ...
                            <small class="text-muted">{{ number_format(($users->where('is_active', true)->count() / max(1, $users->total())) * 100, 1) }}% کل</small>
                        </div>
...
<script>
    $(function() {
        ...
        // ========== نمایش/مخفی کردن نقش‌های هر شرکت (ویرایش) – اصلاح‌شده ==========
        $(document).on('change', '.edit-company-checkbox', function() {
            const companyId = $(this).val();
            const $rolesWrapper = $(`#edit_company_roles_${companyId}`);
            const $rolesSelect = $rolesWrapper.find('select');

            if ($(this).is(':checked')) {
                $rolesWrapper.slideDown(200);
                // اگر سلکت خالی بود، نقش‌ها را پر کن
                if ($rolesSelect.children('option').length === 0) {
                    const availableRoles = $('#editUserModal').data('available-roles') || {};
                    if (availableRoles[companyId]) {
                        $.each(availableRoles[companyId], function(i, role) {
        ...
        // ========== مودال ویرایش کاربر (با پشتیبانی از نقش‌های هر شرکت) – نسخهٔ جدید ==========
        $(document).on('click', '.edit-user-btn', function() {
            ...
            $('#edit_is_active').prop('checked', isActive == 1 || isActive === true || isActive === '1');
            ...
            const availableRoles = btn.data('available-roles') || {};

            // ذخیره availableRoles روی مودال برای استفاده در رویداد change
            $('#editUserModal').data('available-roles', availableRoles);

            userCompanies.forEach(companyId => {
                $(`#edit_company_${companyId}`).prop('checked', true);
                $(`#edit_company_roles_${companyId}`).show();

                const $rolesSelect = $(`#edit_company_roles_${companyId} select`);
                $rolesSelect.empty();

                // پر کردن سلکت با نقش‌های مجاز (عمومی + اختصاصی این شرکت)
                if (availableRoles[companyId]) {
                    $.each(availableRoles[companyId], function(i, role) {
                        $rolesSelect.append($('<option>', {
                            value: role.id,
                ...
                $rolesSelect.val(roleIds).trigger('change');
            });
        ...
        $('#editUserModal').on('hidden.bs.modal', function() {
            ...
            $('.edit-company-checkbox').prop('checked', false);
            $('.company-roles-wrapper').hide().find('select').empty(); // پاک کردن سلکت‌ها
            $(this).data('available-roles', {}); // ریست دیتا
        });
